    ...
    <div className="relative">
      ...
      >
        ...
        <div className="flex items-start justify-between mb-2 sm:mb-3">
          ...
          {/* Chair icon */}
          <div className={`
            w-8 h-8 sm:w-9 sm:h-9 rounded-lg ${s.iconBg}
            flex items-center justify-center
            shrink-0
          `}>
            <ChairIcon className={`w-4 h-4 sm:w-5 sm:h-5 ${s.iconColor}`} />
          </div>
        ...
        <div className="flex-1 min-w-0">
          {table.status === 'occupied' && table.waiter && (
            <div className="flex items-center gap-1.5 text-xs text-zinc-400 mb-1 truncate">
              <svg className="w-3 h-3 shrink-0" viewBox="0 0 24 24" fill="none"
                stroke="currentColor" strokeWidth="2">
                <path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/>
                <circle cx="12" cy="7" r="4"/>
              </svg>
              <span className="truncate">Waiter: {table.waiter}</span>
            </div>
          )}

          {table.status === 'available' && (
            <div className="flex items-center gap-1.5 text-xs text-green-400/80 mb-1 truncate">
              <svg className="w-3 h-3 shrink-0" viewBox="0 0 24 24" fill="none"
