...
const StatCard = ({ label, value, color, bgIcon }) => (
  <div className="
    ...
    rounded-xl p-4 sm:p-5 flex items-end
    justify-between overflow-hidden relative
    min-h-24 sm:min-h-27.5
  ">
    {/* Content */}
    <div className="relative z-10">
      <p className="text-[10px] sm:text-[11px] font-semibold text-zinc-500
        uppercase tracking-widest mb-2 sm:mb-3">
        {label}
      </p>
      <p className={`font-heading text-3xl sm:text-5xl font-bold ${color} leading-none`}>
        {String(value).padStart(2, '0')}
      </p>
    </div>

    {/* Background icon */}
    <div className="absolute right-4 bottom-2 opacity-10 text-5xl sm:text-7xl select-none">
      {bgIcon}
    </div>
  </div>
)
  ...
  return (
    <div className="flex flex-col gap-5 sm:gap-6 w-full">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          <h2 className="font-heading text-2xl sm:text-3xl font-bold text-zinc-100 tracking-tight">
            Floor Map
          </h2>
          <p className="text-xs sm:text-sm mt-1.5 flex flex-wrap gap-x-1.5">
      ...
      {/* ── Floor Sections ── */}
      {isLoading ? (
        <div className="grid grid-cols-[repeat(auto-fill,minmax(140px,1fr))] sm:grid-cols-[repeat(auto-fill,minmax(220px,1fr))] gap-3 sm:gap-4">
      ...
      {!isLoading && filteredTables.length === 0 && (
        <div className="
          ...
          py-16 sm:py-20 gap-4 text-center px-4
          bg-[#1a1208] border border-white/5
          rounded-2xl
        ">
